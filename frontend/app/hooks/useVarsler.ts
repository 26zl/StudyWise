...
export type VarslingTab = "alle" | "frister" | "oppgaver" | "kunngjøringer" | "hendelser";
...
export function useVarslerStateSync(authReady: boolean, serverState?: VarslerState) {
  ...
  const normalizedServerState = useMemo(() => normalizeVarslerState(serverState), [serverState]);
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      persistVarslerState(normalizedLocalState, {
        onSuccess: (data) => {
          const persistedState = normalizeVarslerState(data.varslerState ?? normalizedLocalState);
          lastSyncedSignatureRef.current = createVarslerStateSignature(persistedState);
        },
      });
    }, retryDelayMs);
...
export function useVarsler(harCanvasToken: boolean): UseVarslerResult {
  ...
  const ikkeManuelleOppgaver = useMemo(
    () =>
      (assignmentsQuery.data ?? []).filter(
        (o) => !ferdigeIdSet.has(o.id) && (!o.course_id || !hiddenSet.has(o.course_id)),
      ),
  ...
  const errors = [assignmentsQuery.error, announcementsQuery.error, eventsQuery.error].filter(
    Boolean,
  );
...
export function useVarslingerSide(harCanvasToken: boolean): UseVarslingerSideResult {
  const announcementsQuery = useCanvasAnnouncements(harCanvasToken);
  const coursesQuery = useCanvasCourses(harCanvasToken);
  ...
  const kalenderElementer = useMemo(
    () =>
      (calendarQuery.data?.assignments ?? []).filter(
        (a) => !a.courseId || !hiddenSet.has(a.courseId),
      ),
  ...
  const frister = useMemo(
    () =>
      oppgaver.filter(
        (oppgave) =>
          !oppgave.erInnlevert && oppgave.timerIgjen > 0 && oppgave.timerIgjen <= FRIST_VINDU_TIMER,
      ),
    [oppgaver],
  );
  ...
  const kunngjøringer = useMemo(
    () => buildKunngjøringer(filtrerteAnnouncementsSide, emneNavnMap),
    [filtrerteAnnouncementsSide, emneNavnMap],
  );
  const hendelser = useMemo(() => buildKalenderHendelser(hendelsesElementer), [hendelsesElementer]);
  const alleElementer = useMemo(
  ...
  const errors = [announcementsQuery.error, coursesQuery.error, calendarQuery.error].filter(
    Boolean,
  );
  const hasAnyData = oppgaver.length > 0 || kunngjøringer.length > 0 || hendelser.length > 0;
  const error = errors[0] ?? null;
  const isError = errors.length > 0 && !hasAnyData;
  const hasPartialError = errors.length > 0 && hasAnyData;
  ...
  return {
    ...
    lestIds,
    markAllAsLest,
    isLoading: announcementsQuery.isLoading || coursesQuery.isLoading || calendarQuery.isLoading,
    isError,
    hasPartialError,
    error,
    isHydrated,
...
export function useVarslerPopups(harCanvasToken: boolean, options: UseVarslerPopupsOptions = {}) {
  ...
  const { ulesteCount, alleElementer, lestIds, markAllAsLest, isLoading, isError, isHydrated } =
  ...
  useEffect(() => {
    ...
    try {
      const timeoutId = setTimeout(() => {
        ...
        showToast.info(melding, t("notifications.toast.description"), {
          duration: 6000,
