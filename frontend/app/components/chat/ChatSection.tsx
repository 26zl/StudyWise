...
const ALLOWED_CHAT_MODEL_IDS = new Set(["claude-sonnet-4-6", "claude-haiku-4-5"]);
...
const actionBtnClass =
  "p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors rounded-md hover:bg-slate-100 dark:hover:bg-slate-800";
...
function lagTilkoblingsBanner(
  error: Error | null | undefined,
): { melding: string; type: FeilMeldingType } | null {
  if (!error) return null;
  return getKIBannerForError(error);
}

function hentKontoFornavn(
  input:
    | {
...
export function ChatSection() {
  ...
  const harValgtCanvasData =
    canvasContextSelection.announcements ||
  ...
  const settAktivSamtale = useCallback(
    (chatId: string | null) => {
    ...
    const timeoutId = window.setTimeout(() => {
      startTransition(() => {
        setRenderedMessageCount((current) =>
          Math.min(meldinger.length, current + HISTORY_RENDER_BATCH_SIZE),
        );
      });
    }, 32);
  ...
  const lagreSamtale = useCallback(
    async (oppdatert: Melding[], title?: string) => {
      ...
      const createPromise = saveChat(payload, undefined, titleFromFirst)
        .then((nyId) => {
  ...
  const oppdaterPendingChatId = useCallback(
    (requestId: string, chatId: string | undefined) => {
      if (!chatId) return;
  ...
  const hydratePendingConversation = useCallback(
  ...
  const retryFailedPendingConversation = useCallback(
    async (pending: PendingConversationState) => {
  ...
  const håndterFiler = useCallback(
    (filer: File[]) => {
      ...
      for (const item of items) {
        if (item.type.startsWith("image/")) {
          ...
          if (fil) {
            ...
            bildeFiler.push(
              new File([fil], `innlimt-bilde-${ts}-${bildeFiler.length + 1}.${ext}`, {
                type: fil.type,
              }),
            );
          }
  ...
  const animerTekst = useCallback(
    (id: string, fullText: string, onDone: () => void) => {
      ...
      animationIntervalRef.current = setInterval(() => {
        ...
        settMeldinger((prev) =>
          prev.map((m) => (m.id === id ? { ...m, innhold: chunks.slice(0, index).join("") } : m)),
        );
  ...
  const sendMelding = async ({
    ...
    const brukerMeldingInnhold =
      trimmetTekst || (harVedlegg ? t("chat.analyzeDocumentFallback", { name: vedlagtNavn }) : "");
    ...
    if (harVedlegg) {
      ...
      settVedlegg([]);
      const titleFromFirst =
        brukerMeldingInnhold.trim().slice(0, 50) || t("chat.newConversationFallback");
      ...
      analyserDokument(filTilAnalyse, brukerMeldingInnhold || t("chat.summarizeDocumentFallback"), {
        onSuccess: (data) => {
          ...
          if (!responseText) {
            ...
            if (isMountedRef.current) {
              settMeldinger((prev) =>
                prev.map((m) => (m.id === brukerMelding.id ? { ...m, feilet: true } : m)),
              );
            }
        ...
        onError: (error) => {
          ...
          showToast.error(
            t("chat.documentAnalysisFailed"),
            lagFeilTekst(error instanceof Error ? error : new Error(String(error)), "dokument"),
          );
    ...
    const spørOmKunngjøringer = /kunngjør|announcement|beskjed|melding fra foreleser/i.test(
      brukerMeldingInnhold,
    );
    const spørOmEmner = /emne|course|fag|kurs(?!gjøring)/i.test(brukerMeldingInnhold);
    const spørOmOppgaver = /oppgave|assignment|innlevering|frist|deadline|todo|gjøremål/i.test(
      brukerMeldingInnhold,
    );
        ...
        if (isMountedRef.current && skalOppdatereSynlig) {
          ...
          animerTekst(assistantId, responseText, () => {
            ...
            if (isMountedRef.current) {
              ...
              settMeldinger((prev) => prev.map((m) => (m.id === assistantId ? sisteMelding : m)));
              settSkriver(false);
            }
          });
        } else {
  ...
  const aktiverKunnskapsbase = useCallback(
    (baseName: string) => {
  ...
  useEffect(() => {
    ...
    if (loading || chats.length === 0 || aktivChatId || selectedChatId || meldinger.length > 0)
      return;
    try {
  ...
  }, [animerendeMeldingId, analyserarDokument, hentSynligPendingConversation, meldinger, skriver]);
  ...
  useEffect(() => {
    ...
    if (chat && isMountedRef.current) {
      ...
      const pendingForChat: PendingConversationState | null =
        globalPending?.chatId === chat.id
          ? globalPending
      ...
      const messagesToShow = pendingForChat
        ? [
            ...
            ...(pendingForChat.assistantMessage
              ? [hydrateMelding(pendingForChat.assistantMessage)]
              : []),
          ]
        : mapSavedChatMessages(chat.id, chat.messages);
      brukerErVedBunnRef.current = true;
      settMeldinger(messagesToShow);
  ...
  useEffect(() => {
    ...
    if (!chat) {
      const pending = pendingChatRef.current;
      if (
        (pending && pending.chatId === aktivChatId) ||
        pendingConversationState?.chatId === aktivChatId
      )
        return;
    ...
    if (skriver || pendingChatRef.current || pendingConversationState?.chatId === aktivChatId)
      return;
  ...
  const delingsTittel =
    ...
    meldinger
      .find((melding) => melding.rolle === "user")
      ?.innhold?.trim()
      .slice(0, 50) ||
    t("chat.conversationFallback");
  const panelMelding = kildePanelMeldingId
    ? meldinger.find(
        (melding) => melding.id === kildePanelMeldingId && melding.rolle === "assistant",
      )
    : null;
  ...
  const handleKildeKlikk = useCallback(
    (kilde: import("common/ki").KIChatSource) => {
      const harSourceUrl =
      ...
      const erCanvasFil =
        kilde.sourceKind === "canvas_file" || (kilde.sourceKind === undefined && !harSourceUrl);
  ...
  const renderKildeListe = useCallback(
    (keySuffix = "") => (
      <>
  ...
  const opprettDelingslenke = useCallback(async () => {
    ...
    try {
      ...
      if (!chatId && meldingerRef.current.length > 0) {
        const titleFromFirst =
          meldingerRef.current
            .find((melding) => melding.rolle === "user")
            ?.innhold?.trim()
            .slice(0, 50) || t("chat.newConversationFallback");
        const savedChatId = await saveChat(
      ...
      const meldingTekst =
        error instanceof Error ? error.message : t("chat.couldNotShareChatFallback");
    ...
    <div className="h-full flex min-h-0 overflow-hidden">
        ...
        >
          <div className="max-w-235 mx-auto space-y-5">
            ...
            {mounted && (loading || bytterSamtale) && (
              <div className="py-12">
                <LoadingView
                  text={t(bytterSamtale ? "common.loading.dashboard" : "chat.loadingChatHistory")}
                  fullPage={false}
                />
              </div>
            )}
            ...
            {!bytterSamtale &&
              visibleMeldinger.map((melding, meldingIdx) =>
                (() => {
                  const visbareKilder =
                    melding.rolle === "assistant" ? hentVisbareKilder(melding) : [];
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={melding.rolle === "user" ? "max-w-[80%]" : "w-full min-w-0"}>
                        ...
                        {melding.rolle === "assistant" &&
                          animerendeMeldingId !== melding.id &&
                          !skriver && (
                              ...
                              <div className="flex items-center gap-0.5">
                                {harKilder && (
                                  <button
                                    type="button"
                                    onClick={() => {
                                      setKildePanelMeldingId((prev) =>
                                        prev === melding.id ? null : melding.id,
                                      );
                                    }}
                                  ...
                                  >
                                    ...
                                    <span>
                                      {visbareKilder.length} {t("chat.sourcesPillLabel")}
                                    </span>
                                  </button>
                                )}
                                <button
                                  ...
                                  return (
                                    <button
                                      ...
                                      onClick={async () => {
                                        ...
                                        const sporsmal =
                                          forrige?.rolle === "user" ? forrige.innhold : undefined;
            ...
            {((skriver &&
              (!animerendeMeldingId ||
                !meldinger.some((m) => m.id === animerendeMeldingId && m.innhold.length > 0))) ||
              analyserarDokument) && (
              <div
                        ...
                        {kbMenuOptions.map((option, index) => {
                          ...
                          if (option.type === "deactivate") {
                            return (
                              ...
                              >
                                <span
                                  className={`text-xs font-semibold ${erValgt ? "text-white" : "text-blue-500"}`}
                                >
                                  {option.label}
                                </span>
                          ...
                          return (
                            ...
                            >
                              <Database
                                className={`h-4 w-4 shrink-0 ${erValgt ? "text-white" : "text-blue-500"}`}
                              />
      ...
      {visKildePanel && (
        <aside className="hidden lg:flex w-90 shrink-0 border-l border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 min-h-0 flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">{renderKildeListe()}</div>
        </aside>
      )}
      {visKildePanel && (
        <div
        ...
        >
          ...
          >
            ...
            <div className="overflow-y-auto p-3 space-y-2">{renderKildeListe(":mobile")}</div>
          </div>
        </div>
      )}
    </div>
