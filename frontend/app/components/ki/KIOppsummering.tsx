...
export function KIOppsummering({ tekst, storrelse, variant = "default" }: KIOppsummeringProps) {
  ...
  const renderIkon = (SpinnerEllerIkon: typeof Loader2 | typeof Sparkles) => {
    if (storrelse === "sm") {
      return (
        <SpinnerEllerIkon
          className={`${s.ikonKlasse} ${SpinnerEllerIkon === Loader2 ? spinnerClass : ""}`}
        />
      );
    }
    const str = storrelse === "md" ? 14 : 16;
    return (
      <SpinnerEllerIkon size={str} className={SpinnerEllerIkon === Loader2 ? spinnerClass : ""} />
    );
  };
  ...
  const renderSjekkIkon = () => {
    if (storrelse === "sm") {
      return (
        <CheckCircle2 className="w-3 h-3 text-purple-500 dark:text-purple-400 mt-0.5 shrink-0" />
      );
    }
    return (
  ...
  const knappTekst = resultat
    ? erUtløpt
      ...
      : åpen
        ...
        : labels.showSummary
    : harTekst
      ? labels.summarizeWithAi
      : labels.noContentToSummarize;
  ...
  return (
    <div className={wrapperClass}>
      ...
      {åpen && resultat && (
        <div
          className={`mt-2 sm:mt-3 rounded-lg bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 ${s.boks}`}
        >
          {resultat.oppsummering && (
            <div>
              <h4
                className={`${s.overskrift} font-semibold text-purple-600 dark:text-purple-400 uppercase tracking-wide mb-0.5 sm:mb-1`}
              >
            ...
            <div>
              ...
              <ul className="space-y-1 sm:space-y-1.5">
                {resultat.handlinger.map((punkt, i) => {
                  ...
                  const forklaring =
                    medForklaring.length > 1 ? medForklaring.slice(1).join(" – ").trim() : null;
                  return (
                    <li
                      key={i}
                      className={`flex items-start gap-1.5 sm:gap-2 ${s.tekst} text-slate-700 dark:text-slate-300`}
                    >
                      {renderSjekkIkon()}
                      <span>
