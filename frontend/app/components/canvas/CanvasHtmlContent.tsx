...
function TilpassetBilde({
  ...
  return (
    <span className="relative my-3 inline-block overflow-hidden rounded-lg bg-slate-100 dark:bg-slate-800">
      {laster && <span className="absolute inset-0 animate-pulse bg-slate-200 dark:bg-slate-700" />}
      {resolvedSrc ? (
        <img
          src={resolvedSrc}
          alt={alt}
        ...
        <span
          className={`inline-flex min-h-24 min-w-32 items-center justify-center px-4 py-3 text-sm ${bildeFeilet ? "text-red-500 dark:text-red-400" : "text-slate-500 dark:text-slate-400"}`}
        >
...
export function CanvasHtmlContent({
  html,
  className,
  canvasBaseUrl,
}: CanvasHtmlContentProps): JSX.Element {
  const { t } = useLanguage();
  ...
  return <div className={className}>{parseCanvasHtml(html, parser)}</div>;
}
