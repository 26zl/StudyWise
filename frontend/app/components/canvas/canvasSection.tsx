    ...
    <div className="space-y-4">
      ...
      {totalElementer > PAGE_SIZE && (
        <div className="flex items-center justify-between pt-4 text-sm text-slate-500 dark:text-slate-400">
          <span>{labels.shownOfTotal({ shown: antallVist, total: totalElementer })}</span>
          {harFlereÅHente && (
            <button
              type="button"
...
function EmneVisning({ harCanvasToken }: { harCanvasToken: boolean }) {
  ...
  const [valgtEmneVisning, settValgtEmneVisning] = useState<
    "modules" | "files" | "frontpage" | "pages" | "announcements"
  >("frontpage");
  ...
  const skjulEmne = useCallback(
    (courseId: number) => {
      const nyeIds = Array.from(new Set([...hiddenIds, courseId]));
      oppdaterHiddenCourses.mutate(
        { courseIds: nyeIds },
        {
  ...
  const emneKunngjoringerQuery = useCanvasCourseAnnouncements(valgtEmneId, valgtEmneAktivert);
      ...
      <div>
          ...
          return (
            <div className="mb-4 flex flex-wrap items-center gap-2">
              ...
              {visModulerTab && (
                ...
                >
                  {labels.modules}
                  {meta?.modulesCount ? ` (${meta.modulesCount})` : ""}
                </button>
              )}
              {visSiderTab && (
                <button
                ...
                >
                  {labels.pages}
                  {meta?.pagesCount ? ` (${meta.pagesCount})` : ""}
                </button>
              )}
              {visFilerTab && (
                <button
                ...
                >
                  {labels.files}
                  {meta?.filesCount ? ` (${meta.filesCount})` : ""}
                </button>
              )}
              <button
                type="button"
        ...
        {valgtEmneVisning === "frontpage" && (
          <div className="space-y-3">
            ...
            {frontPageQuery.isError && (
              <FeilMelding
                melding={lagBrukervennligFeilmelding(
                  frontPageQuery.error instanceof Error ? frontPageQuery.error : null,
                  { canvas: true },
                  labels.frontPageLoadError,
                )}
              />
            )}
            {frontPageQuery.data && (
        ...
        {valgtEmneVisning === "modules" && (
          <>
            ...
            {modulerQuery.isError && <FeilMelding melding={labels.modulesAccessError} />}

            {/* Vis melding når query er disabled pga metadata sier ingen moduler */}
            {!modulerQuery.isLoading &&
              !modulerQuery.isError &&
              !modulerQuery.data?.modules?.length &&
              metaReady &&
              valgtMeta?.hasModules === false && (
                    ...
                    <div className="px-4 pt-1 pb-2">
                      <CanvasKIHandlinger
                        tekst={[
                          ...
                          module.items?.length
                            ? `${labels.contentLabel}: ${module.items
                                .map((it) => it.title)
                                .filter(Boolean)
                                .join(", ")}.`
                            : labels.noItems,
                          ...
                          if (isFile && !downloadPath && valgtEmneId) {
                            ...
                            try {
                              ...
                              if (result.type === "File" && result.downloadPath) {
                                ...
                                const pathMatch = /^\/api\/canvas\/filer\/(\d+)\/download$/.exec(
                                  result.downloadPath,
                                );
        ...
        {valgtEmneVisning === "pages" && (
          <div className="space-y-3">
            {siderQuery.isLoading && <LoadingView text={labels.pagesLoading} fullPage={false} />}
            {siderQuery.isError && <FeilMelding melding={labels.pagesAccessError} />}
            {!siderQuery.isLoading &&
              !siderQuery.isError &&
              !siderQuery.data &&
              metaReady &&
              valgtMeta?.hasPages === false && (
                <p className="text-sm text-slate-500 dark:text-slate-400">{labels.pagesEmpty}</p>
              )}
            {siderQuery.data && siderQuery.data.length === 0 && (
              <p className="text-sm text-slate-500 dark:text-slate-400">
                {labels.pagesNothingToShow}
              </p>
            )}
            {siderQuery.data && siderQuery.data.length > 0 && (
              <div className="rounded-xl border border-slate-200 dark:border-slate-700">
  ...
  return (
    <div className="space-y-4">
      ...
      {aktivTab === "hidden" && (
        <>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {labels.hiddenCoursesDescription}
          </p>
          {skjulteEmner.length === 0 ? (
            <div className="text-center py-12">
          ...
          {synligeEmner.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {synligeEmner.map((emne) => {
                ...
                const åpneEmne = (
                  visning: "frontpage" | "modules" | "files" | "pages" | "announcements",
                ) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      aria-label={`${labels.contentLabel} ${emne.name}`}
                    >
                      {metadataLaster ? (
                        <>
...
function TokenUgyldigAdvarsel() {
  return <CanvasTokenNotice variant="invalid" className="mx-4 mt-4 md:mx-6" />;
}

// Hovedkomponent
export function CanvasSection({
  startVisning = "announcements",
      ...
      <div className="min-h-full">
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          {canvasTokenInvalid ? null : (
            <>
              {visning === "announcements" && (
                <KunngjoringVisning harCanvasToken={harCanvasToken} />
              )}
...
function OppgaverVisning({ harCanvasToken }: { harCanvasToken: boolean }) {
  ...
  const [filter, settFilter] = useState<
    "alle" | "kommende" | "forfalt" | "uten-frist" | "innlevert"
  >("kommende");
  ...
  const filtrerteOppgaver = useMemo(() => {
    ...
    if (sortering === "frist") {
      ...
      filtrert.sort((a, b) =>
        a.course_name.localeCompare(b.course_name, language === "en" ? "en" : "nb"),
      );
    }
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/50 divide-y divide-slate-200 dark:divide-slate-700">
        {synligeOppgaver.map((assignment) => {
          ...
          const dagerTekst = fristDato
            ? formaterDagerRelativtFrist(dagerFraIdag(fristDato), language)
            : "";
          ...
          const oppsummeringstekst = [
            ...
            assignment.points_possible != null
              ? `${labels.points}: ${assignment.points_possible}`
              : "",
            assignment.due_at
              ? `${labels.due}: ${formaterDatoLong(assignment.due_at, language)}`
              : "",
          ]
            .filter(Boolean)
            .join(". ");
                ...
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  {!erInnlevert && (
                    <button
                      ...
                      aria-label={
                        erManueltFerdig ? labels.unmarkAsSubmitted : labels.markAsSubmitted
                      }
                  ...
                  <div className="flex-1 min-w-0">
                    <h3
                      className={`font-medium text-sm sm:text-base truncate ${erManueltFerdig ? "line-through text-slate-400 dark:text-slate-500" : "text-slate-900 dark:text-white"}`}
                    >
                      {assignment.name}
                    </h3>
                    <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1">
                      ...
                      {assignment.points_possible != null ? (
                        <span>
                          · {assignment.points_possible} {labels.pointsSuffix}
                        </span>
                      ) : null}
                      {erInnlevert && (
