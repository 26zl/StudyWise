...
export function getCanvasTenantCachePrefix(baseUrl: string): string {
  return crypto
    .createHash("sha256")
    .update(normalizeCanvasBaseUrl(baseUrl))
    .digest("hex")
    .slice(0, 12);
}
...
export const erInnenforKalenderVindu = (
  isoDato?: string | null,
  options?: {
    månederTilbake?: number;
    månederFrem?: number;
...
export async function hentCanvasData<T>(
  ...
  const baseUrl = options.baseUrl ? normalizeCanvasBaseUrl(options.baseUrl) : undefined;
  ...
  if (erSensitiv) {
    return canvasCircuit.execute(() =>
      hentCanvasDataImpl<T>(endpoint, options, cleanToken, baseUrl),
    );
  }
  ...
  const promise = canvasCircuit
    .execute(() => hentCanvasDataImpl<T>(endpoint, options, cleanToken, baseUrl))
    .finally(() => {
...
async function hentCanvasDataImpl<T>(
    ...
    try {
      ...
      if (cachedData) {
        ...
        if (parsed.error && parsed.cached) {
          logger.info(
            { cacheKey: cacheNokkel, errorCode: parsed.error },
            "Redis Cache HIT (negativ)",
          );
  ...
  while (currentUrl && pagesFetched < maxPages) {
    if (seenUrls.has(currentUrl)) {
      logger.warn(
        { endpoint, currentUrl, pagesFetched },
        "Canvas paginering: samme URL sett to ganger — bryter loop",
      );
      break;
    }
      ...
      try {
        ...
        if (response.status === 429) {
          ...
          if (retryCount >= maxRetries) {
            const error = new Error(
              "Canvas rate limit nådd - maks antall retry forsøk brukt",
            ) as CanvasHttpError;
            error.status = 429;
        ...
        if (!response.ok) {
          ...
          if (!erSensitiv && cacheTtl > 0 && permanentErrors.includes(errorCode)) {
            ...
            logger.debug(
              { endpoint, errorCode, cacheTtl: negativeCacheTtl },
              "Cachet negativ respons",
            );
          }
          ...
          const error = createCanvasError(errorCode, getErrorMessage(errorCode), {
            httpStatus: response.status,
            endpoint,
            details: errorText,
            retryAfter,
  ...
  if (!erSensitiv && cacheTtl > 0) {
    await setCache(cacheNokkel, JSON.stringify(result), cacheTtl);
    logger.info(
      { endpoint, pagesFetched, itemsCount: allItems.length, cacheTtl },
      "Canvas data hentet og cachet",
    );
  }
  return result;
}
...
export function validateCanvasRedirectUrl(
  urlStr: string,
