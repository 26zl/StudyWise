...
export function enforceSvarKilde(
  ...
  if (modelEmittedKilde === "canvas" && !ctx.harCanvasMetadata && !ctx.harKursmateriale) {
    if (ctx.harKunnskapsbase || ctx.harLiveUrl) return "kunnskapsbase";
    return "generell";
  }
...
export async function chatCompletion(options: {
  ...
  const shouldFallbackToDefaultModel = (error: unknown): boolean => {
    ...
    const responseBody =
      typeof maybeError?.responseBody === "string" ? maybeError.responseBody : "";
    const causeResponseBody =
      typeof maybeError?.cause?.responseBody === "string" ? maybeError.cause.responseBody : "";
    const message = typeof maybeError?.message === "string" ? maybeError.message : "";
    const causeMessage =
      typeof maybeError?.cause?.message === "string" ? maybeError.cause.message : "";
    ...
    return (
      (primaryStatus === 404 || causeStatus === 404 || hasNotFoundSignal) && model !== DEFAULT_MODEL
    );
  };
...
export function recordAnthropicCreditFailure(): void {
  ...
  logger.warn("Anthropic credit-failure registrert — helsesjekk rapporterer down i 15 min");
}
...
export async function chatCompletionWithVision(options: {
  ...
  const { model, messages, images, max_tokens, temperature, signal, traceName, traceMeta } =
    options;

  if (!anthropicClient) {
    throw new Error(
      "Vision er ikke tilgjengelig. Kall kun chatCompletionWithVision når isVisionAvailable(model) er sann.",
    );
  }
  ...
  try {
    let result = await anthropicCircuit.execute(() =>
      callAnthropicWithVision({
        model,
        messages,
        images,
        max_tokens,
...
async function callAnthropic(options: {
  ...
  const isModelNotFoundError = (error: unknown): boolean => {
    const maybeError = error as
      | { status?: number; responseBody?: string; message?: string }
      | undefined;
    const responseBody =
      typeof maybeError?.responseBody === "string" ? maybeError.responseBody : "";
    const message = typeof maybeError?.message === "string" ? maybeError.message : "";
  ...
  logger.info(
    { model, messageCount: sdkMessages.length },
    "Sender til Anthropic Claude (Vercel AI SDK)",
  );
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const { text, fallbackUsage } = await raceMedTimeout(
        Promise.all([streamResult.text, streamResult.totalUsage]).then(([t, u]) => ({
          text: t,
          fallbackUsage: u as CapturedUsage,
        })),
      ...
      const durationMs = Date.now() - startMs;
      logger.warn({ model, durationMs, err: error }, "Anthropic Claude-kall feilet");
      throw error;
    }
  }
...
async function callAnthropicWithVision(options: {
  ...
  const anthropicMessages: AnthropicMessage[] = nonSystemMessages.map((m, idx) => {
    ...
    const erSisteUserMelding = m.role === "user" && idx === nonSystemMessages.length - 1;
