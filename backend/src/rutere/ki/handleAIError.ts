...
export function classifyAIError(
  ...
  if (options?.timeoutLabel && errorMessage.includes(options.timeoutLabel)) {
    return {
      category: "timeout",
      userMessage: options.timeoutMessage ?? "Forespørselen tok for lang tid. Prøv igjen.",
      status: 504,
    };
  }
  ...
  if (
    ...
    return {
      category: "auth_error",
      userMessage: "KI-tjenesten er ikke konfigurert riktig akkurat nå. Kontakt administrator.",
      status: 503,
    };
  }
...
export function handleAIError(
  ...
  res.status(classified.status).json(parseErrorResponse(schema, classified.userMessage));
  return true;
}
...
export function handleAIJsonRouteError(
  ...
  if (error instanceof ZodError || isJsonSyntaxError || options.invalidResponseTest?.(error)) {
    apiError.badRequest(res, options.invalidResponseMessage ?? "KI-responsen kunne ikke tolkes");
    return true;
  }
  ...
  const classified = classifyAIError(error, {
    timeoutLabel: options.timeoutLabel,
    timeoutMessage: options.timeoutMessage ?? "Genereringen tok for lang tid. Prøv igjen.",
  });
...
export function checkAIClientUnavailable(res: Response, model: string, schema: ZodType): boolean {
  ...
  res
    .status(500)
    .json(parseErrorResponse(schema, "KI-tjenesten er ikke konfigurert. Kontakt administrator."));
  return true;
}
