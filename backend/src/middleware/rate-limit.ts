...
const setRateLimitHeaders = (res: Response, rateRes: RateLimiterRes, limit: number) => {
  ...
  res.setHeader("X-RateLimit-Reset", String(Math.ceil((Date.now() + rateRes.msBeforeNext) / 1000)));
};
  ...
  const middleware = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!useRedis) {
      ...
      if (isProd && failClosedInProd) {
        logger.error(
          { keyPrefix, path: req.path },
          "Rate limiter: Redis utilgjengelig i prod — fail-closed",
        );
      ...
      logger.warn(
        { keyPrefix },
        "Rate limiter: Redis utilgjengelig, faller tilbake til in-memory (per-instans)",
      );
    }
    ...
    try {
      ...
      if (isRateLimiterResult(err)) {
        ...
        audit({
          ...
          logger.error(
            { err: auditErr, keyPrefix, path: req.path },
            "Audit-logging feilet for RATE_LIMIT_EXCEEDED",
          );
        });
        return apiError.rateLimited(
          res,
          "Du har nådd grensen for forespørsler. Prøv igjen senere.",
        );
      }
...
export const rateLimitUsernameCheck = isProd
  ? createRateLimiter({
      points: 10,
      duration: 60,
      keyPrefix: "rlflx:username-check",
      failClosedInProd: true,
    })
  : createRateLimiter(devMeLimit);
