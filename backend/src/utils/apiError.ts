...
export function sendUnknownError(
  ...
  logger.error({ err: error, ...context }, `Ukjent feil: ${errorMessage.substring(0, 100)}`);
...
export const apiError = {
  // Autentisering
  unauthorized: (res: Response, melding?: string) => sendError(res, "auth_error", { melding }),
  ...
  // Konflikt
  conflict: (res: Response, melding: string) => sendError(res, "conflict", { melding }),
