...
function sanitizeNotionErrorBody(body: string): string {
  ...
  return truncated.replace(/(secret_[A-Za-z0-9_-]+|Bearer\s+[A-Za-z0-9._-]+)/gi, "[redacted]");
}
...
function mapBlock(block: ExportBlock): NotionBlock[] {
  switch (block.type) {
    ...
    case "table": {
      ...
      const colCount = hasHeaders ? block.headers!.length : (block.rows[0]?.length ?? 0);
...
async function createNotionPage(
  ...
  if (!createRes.ok) {
    ...
    if (createRes.status === 403) {
      throw new Error(
        "Notion har avvist tilgang. Del parent-siden med integrasjonen din i Notion.",
      );
    }
    if (createRes.status === 404) {
      throw new Error(
        "Fant ikke Notion-siden. Sjekk at side-ID er riktig og at siden er delt med Notion-integrasjonen din.",
      );
    }
  ...
  for (let i = 0; i < childBlocks.length; i += NOTION_MAX_BLOCKS_PER_REQUEST) {
    ...
    const appendRes = await fetch(`${NOTION_BASE_URL}/blocks/${pageData.id}/children`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${notionApiKey}`,
        "Notion-Version": NOTION_API_VERSION,
...
export const notionProvider: ExportProvider = {
  ...
  async execute(doc: ExportDocument, options?: RuntimeProviderOptions): Promise<ExportResponse> {
    ...
    if (!notionApiKey) {
      throw new Error(
        "Notion-eksport er ikke konfigurert. Legg til Notion API-nøkkel i innstillinger.",
      );
    }

    const parentPageId =
      options?.notion?.parentPageId?.trim() || options?.notion?.defaultPageId?.trim();
    if (!parentPageId) {
      throw new Error(
        "Notion parent page ID er påkrevd. Legg inn standard side-ID i innstillinger eller oppgi den i eksportdialogen.",
      );
    }
