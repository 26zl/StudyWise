...
if (!ArrayBuffer.prototype.transfer) {
  ArrayBuffer.prototype.transfer = function (newByteLength?: number): ArrayBuffer {
    ...
    try {
      ...
      /* allerede frigjort eller ustøttet */
    }
    return newBuf;
  };
}
...
export function validateFileMagicBytes(buffer: Buffer, declaredMimeType: string): string | null {
  ...
  if (
    declaredNorm.startsWith("text/") ||
    declaredNorm === "text/plain" ||
    declaredNorm === "text/markdown" ||
    declaredNorm === "text/csv"
  ) {
    if (fromMagic !== null) {
  ...
  if (
    fromMagic === "application/vnd.openxmlformats-officedocument.wordprocessingml.document" &&
    allowedForZip.includes(declaredNorm)
  ) {
...
function estimateZipDecompressedSize(buffer: Buffer): number | null {
  ...
  while (offset + 30 <= buffer.length) {
    ...
    if (
      buffer[0 + offset] !== 0x50 ||
      buffer[1 + offset] !== 0x4b ||
      buffer[2 + offset] !== 0x03 ||
      buffer[3 + offset] !== 0x04
    ) {
      break;
    }
...
function sanitizeText(text: string): { cleanText: string; redacted: boolean } {
  ...
  const adresseRegex =
    ...
    /\b([A-ZÆØÅ][a-zæøå]+(?:gata|gaten|veien|vegen|vei|gate|plass|allé|alléen|vn\.|gt\.)?)\s+\d{1,4}[A-Za-z]?\b(?:\s*,?\s*\d{4}\s+[A-ZÆØÅ][a-zæøå]+)?/gi;
...
async function performOCR(buffer: Buffer): Promise<{ text: string; confidence: number }> {
  try {
    ...
    logger.info({ textLength: text.length, confidence }, "OCR completed");
  ...
  try {
    ...
    if (!text || text.trim().length === 0) {
      ...
      return {
        ...
        truncated: false,
        error:
          "Kunne ikke finne tekst i bildet. Dette kan skyldes: stilisert/dekorativ tekst, tekst over komplekse bakgrunner, eller lav bildekvalitet. Prøv et bilde med tydelig, vanlig tekst på enkel bakgrunn.",
      };
    }
    ...
    return {
      ...
      truncated: false,
      error:
        "Kunne ikke lese tekst fra bildet. Sørg for at det er et gyldig bilde med lesbar tekst.",
    };
  }
}
...
): Promise<{ text: string; avgConfidence: number; pagesProcessed: number }> {
...
async function parsePdfDocument(
  ...
  try {
    ...
    try {
      ...
      numPages = Number.isFinite(parsed.total) && parsed.total > 0 ? parsed.total : 1;
    } finally {
    ...
    const hasRealText =
      typeof text === "string" && text.trim().length > 0 && /[a-zA-ZæøåÆØÅ0-9]{3,}/.test(text);
    ...
    if (!hasRealText) {
      ...
      if (options?.syncMode && buffer.length > MAX_OCR_FILE_SIZE_SYNC) {
        ...
        return {
          ...
          truncated: false,
          error:
            "PDF-filen inneholder ingen lesbar tekst og er for stor for OCR under synkronisering.",
        };
      }

      logger.warn(
        "PDF inneholder ingen lesbar tekst via standard ekstraksjon, rasteriserer sider for OCR",
      );
      ...
      return {
        ...
        truncated: false,
        error:
          "PDF-filen inneholder ingen lesbar tekst. Verken tekstekstraksjon eller OCR klarte å lese innholdet.",
      };
    }
    ...
    try {
      ...
      if (images.length > 0) {
        logger.info(
          ...
          "Word document has embedded images, running OCR",
        );
        const ocrResult = await ocrImageBuffers(images);
 ...
 */
async function parseTextDocument(buffer: Buffer, fileType: string): Promise<DocumentParseResult> {
  try {
