...
- **APM**: Datadog (`dd-trace`) — required in production via `validateEnv()` and initialized in `backend/src/datadog.ts`; init is wrapped in try/catch so the server can still handle tracer startup failures gracefully. Frontend: RUM runs via `DatadogRum` when `DD_RUM_APPLICATION_ID`/`DD_RUM_CLIENT_TOKEN` or `NEXT_PUBLIC_DD_RUM_APPLICATION_ID`/`NEXT_PUBLIC_DD_RUM_CLIENT_TOKEN` are set.
...
- **RBAC, privacy, retention, observability, account deletion**: See `docs/security/` — `governance-and-operations.md` (summary), `rbac.md`, `data-inventory-and-privacy.md`, `retention-policy.md`, `observability.md`, `account-deletion.md`.
