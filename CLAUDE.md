...
- **OAuth account ID** - Clerk `externalAccounts` provides ID via `providerUserId` (preferred). `externalId` is deprecated — don't use it. `providerUserId` can be empty string `""` for unverified accounts, so use `account.providerUserId?.trim() || account.id?.trim()` as fallback (where `account.id` is the Clerk-internal external account ID).
